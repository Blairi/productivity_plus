...
if($_SERVER["REQUEST_METHOD"] === "POST"){
    ...
    if(empty($errors)){
        $query = "INSERT INTO user (name, last_name, username, email, password, tasks_completed) VALUES ('${name}', '${last_name}', '${user_name}', '${email}', '${password}', 0);";

        
        // Execute query
        mysqli_query($db, $query);
        header("Location: /login.php?alert=sucesfully");
...
<main class="margin-top-bar">
    <header class="header-phrase container">
        <h2 class="center-text">Registrate</h2>
